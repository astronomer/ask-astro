...
def ask_astro_load_slack():
    ...
    _check_schema = WeaviateCheckSchemaBranchOperator(task_id='check_schema', 
                                                      ...
                                                      follow_task_ids_if_true=["extract_slack_archive",
                                                                               ],
    ...
    slack_docs = task(extract.extract_slack_archive, trigger_rule='none_failed', retries=3).expand(source=slack_channel_sources)

    markdown_tasks = [slack_docs]

    split_md_docs = task(split.split_markdown).expand(df=markdown_tasks)
    
    task.weaviate_import(ingest.import_upsert_data, trigger_rule='none_failed', weaviate_conn_id=_WEAVIATE_CONN_ID).partial(class_name='Docs', primary_key='docLink').expand(dfs=[split_md_docs])
