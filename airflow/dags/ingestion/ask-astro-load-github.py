...
def ask_astro_load_github():
    ...
    _check_schema = WeaviateCheckSchemaBranchOperator(task_id='check_schema', 
                                                      ...
                                                      follow_task_ids_if_true=["extract_github_markdown",
                                                                               "extract_github_rst",
    ...
    md_docs = task(extract.extract_github_markdown, 
                   trigger_rule='none_failed', 
                   retries=3)\
                .partial(github_conn_id=_GITHUB_CONN_ID)\
                .expand(source=markdown_docs_sources)
    
    rst_docs = task(extract.extract_github_rst, 
                    trigger_rule='none_failed', 
                    retries=3)\
                .partial(github_conn_id=_GITHUB_CONN_ID, 
                         rst_exclude_docs=rst_exclude_docs)\
                .expand(source=rst_docs_sources)
    
    issues_docs = task(extract.extract_github_issues, 
                       trigger_rule='none_failed', 
                       retries=3)\
                    .partial(github_conn_id=_GITHUB_CONN_ID)\
                    .expand(source=issues_docs_sources)
    
    code_samples = task(extract.extract_github_python, 
                        trigger_rule='none_failed', 
                        retries=3)\
                    .partial(github_conn_id=_GITHUB_CONN_ID)\
