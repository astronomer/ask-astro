...
def ask_astro_load_bulk():
    ...
    _check_object_count = check_object_count(weaviate_doc_count=weaviate_doc_count, 
                                             class_name='Docs')

    md_docs = extract_github_markdown\
                    .expand(source=markdown_docs_sources)
    
    rst_docs = extract_github_rst\
                    .expand(source=rst_docs_sources)
    
    issues_docs = extract_github_issues\
                    .expand(source=issues_docs_sources)
    
    stackoverflow_docs = extract_stack_overflow\
                    .partial(stackoverflow_cutoff_date=stackoverflow_cutoff_date)\
                    .expand(tag=stackoverflow_tags)
    
    slack_docs = extract_slack\
                    .expand(source=slack_channel_sources)
    ...
    task.weaviate_import(ingest.import_data, 
                         weaviate_conn_id=_WEAVIATE_CONN_ID)\
...
def test():
    ...
    WeaviateCreateSchemaOperator(task_id='create_schema', 
                                 weaviate_conn_id=_WEAVIATE_CONN_ID,
                                 class_object_data='file://include/data/schema.json',
                                 existing='fail').execute(context={})
    
    WeaviateHook(_WEAVIATE_CONN_ID).get_conn().query.aggregate(class_name='Docs').with_meta_count().do()
    
    for df in [pd.concat([extract.extract_github_markdown(source) for source in markdown_docs_sources]),
        ...
        WeaviateImportDataOperator(task_id='test', 
                                   ...
                                   data=df, 
                                   uuid_column='uuid', 
                                   weaviate_conn_id=_WEAVIATE_CONN_ID).execute(context={})
    
    for df in [pd.concat([extract.extract_github_python(source) for source in code_samples_sources]),
    ...
    search = weaviate_client.query\
        ...
        .with_where({"path": ["docSource"], 
                     "operator": "Equal", 
                     "valueText": "astronomer registry modules"}).do()
