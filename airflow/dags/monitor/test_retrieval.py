...
def test_retrieval(question_number_subset: str):
    ...
    def create_drive_folders(drive_folder:str) -> list:
        ...
        parent_id = get_or_create_drive_folder(
                gd_hook=gd_hook, folder_name=drive_folder_parts[0], parent_id=None)
        
        folder_id = get_or_create_drive_folder(
                gd_hook=gd_hook, folder_name=drive_folder_parts[1], parent_id=parent_id)

        gd_hook.get_conn().permissions().create(
            fileId=parent_id, 
            body={
                    "type": "domain",
                    "domain": google_domain_id,
                    "role": "writer"
                }).execute()
    ...
    def download_test_questions(test_questions_sheet_id: str):
        ...
        if values:
            pd.DataFrame(values[1:], columns=values[0])\
                .to_csv(test_question_template_path, index=False)
    ...
    def upload_results(results_file: str, drive_id: str, ts_nodash: str = None):
        ...
        results_sheet = gd_hook.get_conn().files().create(
            body={
        ...
        existing_data = gs_hook.get_values(
            spreadsheet_id=results_sheet.get("id"),
            range_="A1")
    ...
    _upload_results = upload_results(
        results_file=_results_file, drive_id=_results_folder_id)
